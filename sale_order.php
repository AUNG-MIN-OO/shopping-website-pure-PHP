...
//insert into sale_order table
    $stmt = $pdo->prepare("INSERT INTO orders (user_id,total_price,order_date) VALUES (:user_id,:total,:order_date)");
    $result =$stmt->execute(
        array(':user_id'=>$user_id,':total'=>$total,':order_date'=>date('Y-m-d H:i:s'))
    );
    if ($result){
